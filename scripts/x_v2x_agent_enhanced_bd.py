...
class Xagent(BasicAgent):
    def __init__(self, env, model, dt=0.1, target_speed=30) -> None:
        ...
        self._model = model
        self._original_v = self._model.target_v
    ...
    def run_step(self,lv=None):
        self._model.reset_Q()
        self._model.target_v = self._original_v
        ...
        if len(self._local_planner._waypoints_queue) == 0:
            ...
            for wp in carla_wp:
                ...
                if last_state is not None:
                    if (ref_state[0]-last_state[0])**2+(ref_state[1]-last_state[1])**2 < 0.05:
        ...
        # if the delta yaw from the egovehicle to the waypoint is larger than 45 degree, modify the wp_yaw to be the same direction as the egovehicle
        # if np.abs(wp_yaw - current_state[2]) > np.pi/4:
        #     wp_yaw = current_state[2]
            # print('wp_yaw is modified to be the same direction as the egovehicle')
        ...
        lcrossable_list = [carla.LaneMarkingType.Broken, carla.LaneMarkingType.BottsDots]
        rcrossable_list = [carla.LaneMarkingType.Broken, carla.LaneMarkingType.BottsDots]
        lncrossable_list = [carla.LaneMarkingType.Solid, carla.LaneMarkingType.BrokenSolid, carla.LaneMarkingType.SolidSolid , carla.LaneMarkingType.BrokenBroken, carla.LaneMarkingType.SolidBroken, carla.LaneMarkingType.Curb, carla.LaneMarkingType.Grass, carla.LaneMarkingType.Other]
        rncrossable_list = [carla.LaneMarkingType.Solid, carla.LaneMarkingType.SolidBroken, carla.LaneMarkingType.SolidSolid, carla.LaneMarkingType.BrokenBroken, carla.LaneMarkingType.BrokenSolid, carla.LaneMarkingType.Curb, carla.LaneMarkingType.Grass, carla.LaneMarkingType.Other]
        # lc_bool = lnc_bool = rc_bool = rnc_bool = False
        self._model.Q = self.Q_origin
        vpf_offset = 0.25
        RIGHT_ADDED = False
        LEFT_ADDED = False
        # if the vehicle will cross a junction, set nc_road_pf to both sides of the road
        is_junctions = []
        for i in range(15):
            is_junctions.append(self._local_planner._waypoints_queue[i][0].is_junction)
        if (is_junctions[0] is False) and (True in is_junctions) and lv is not None:
            ...
            # enlarge the lane following cost
            # self._model.Q[0, 0] = 1000
            # self._model.Q[1, 1] = 1000
            # self._model.Q[2, 2] = 1000
            # set the target_v to be 10
            # self._model.target_v = 10
            # print('The vehicle will cross a junction, it should keep in the correct lane!!!')
            # ## apply the virtual PF to the left side of the road1
            ...
            # self._model.solver_add_nc_road_pf([[left_bound+vpf_offset+1.75, 1]], wp_yaw, carla=True)
            # LEFT_ADDED = True
            # apf_nc_road += self._model.nc_road_pf([[left_bound+vpf_offset+1.75 , -1]], ref_traj, wp_yaw)

            # ## apply the virtual PF to the right side of the road
            # # get the waypoint the vehicle is following
            ...
            # # self._model.Q[1, 1] = 10
            # # set a relaxed virtual PF for the unknown lane
            # self._model.solver_add_nc_road_pf([[right_bound-vpf_offset-1.75, -1]], wp_yaw, carla=True)
            # RIGHT_ADDED = True
            # apf_nc_road += self._model.nc_road_pf([[right_bound-vpf_offset-1.75 , -1]], ref_traj, wp_yaw)
            # print('The vehicle will cross a junction, lane changing is prohibited!!!')
        # elif (is_junctions[0] is True) and (True in is_junctions):
        #     # enlarge the lane following cost
        #     self._model.Q[0, 0] = 1000
        #     self._model.Q[1, 1] = 1000
            # self._model.Q[2, 2] = 1000
            # set the target_v to be 10
            # self._model.target_v = 10
            # print('The vehicle is in a junction, it should strictly follow the waypoints!!!')
            ...
            # print('The vehicle is in a junction, lane changing is prohibited!!!')
        # if the vehicle will turn left or right, set nc_road_pf
        elif self._local_planner._waypoints_queue[0][1].value == 2 or self._local_planner._waypoints_queue[5][1].value == 2:
            # if rhe vehicle has crossed the road lane, set ordinary PF
            if self.lat_dis_wp_ev(self._local_planner._waypoints_queue[1][0], self._env.ego_vehicle) < 1.80:
                self._model.solver_add_nc_road_pf([[left_bound, 1]], wp_yaw, carla=True)
                LEFT_ADDED = True
                apf_nc_road += self._model.nc_road_pf([[left_bound, 1]], ref_traj, wp_yaw)
                ...
                # lc_bool = True
                # pass
                if  wp.left_lane_marking.color == carla.LaneMarkingColor.White:
            ...
            # else:
            #     # add non-crossable road PF
            #     self._model.solver_add_nc_road_pf([[left_bound, 1]], wp_yaw, carla=True)
            #     LEFT_ADDED = True
            #     self._model.nc_road_pf([[left_bound, 1]], ref_traj, wp_yaw)
        elif wp.left_lane_marking.type in lcrossable_list:
            # lc_bool = True
            # pass
            if wp.left_lane_marking.color == carla.LaneMarkingColor.White:
        ...
        #     if wp.is_junction:
        #         pass
        #     self._model.solver_add_nc_road_pf([[left_bound+vpf_offset+0.25, 1]], wp_yaw, carla=True)
        #     LEFT_ADDED = True
        #     apf_nc_road += self._model.nc_road_pf([[left_bound+vpf_offset+0.25, 1]], ref_traj, wp_yaw)
        ...
        #     apf_nc_road += self._model.nc_road_pf([[left_bound+vpf_offset, -1]], ref_traj, wp_yaw)
        # if the vehicle will turn left or right, set nc_road_pf
        # if there exists at least one _is_junction waypoint, set nc_road_pf
        if RIGHT_ADDED:
            pass
        elif self._local_planner._waypoints_queue[0][1].value == 1 or self._local_planner._waypoints_queue[5][1].value == 1:
            # if the vehicle has crossed the road lane, set ordinary PF
            if self.lat_dis_wp_ev(self._local_planner._waypoints_queue[0][0], self._env.ego_vehicle) < 1.80:
                self._model.solver_add_nc_road_pf([[right_bound, -1]], wp_yaw, carla=True)
                RIGHT_ADDED = True
                apf_nc_road += self._model.nc_road_pf([[right_bound, -1]], ref_traj, wp_yaw)
            ...
            # rc_bool = True
                # pass
                if wp.right_lane_marking.color == carla.LaneMarkingColor.White:
            ...
            #     self._model.solver_add_nc_road_pf([[right_bound, -1]], wp_yaw, carla=True)
            #     RIGHT_ADDED = True
            #     self._model.nc_road_pf([[right_bound, -1]], ref_traj, wp_yaw)
        elif wp.right_lane_marking.type in rcrossable_list:
            # rc_bool = True
            # pass
            if wp.right_lane_marking.color == carla.LaneMarkingColor.White:
        ...
        #     RIGHT_ADDED = True
        #     self._model.nc_road_pf([[right_bound, -1]], ref_traj, wp_yaw)
        # if RIGHT_ADDED == False:
        #     if wp.is_junction:
        #         pass
        #     self._model.solver_add_nc_road_pf([[right_bound-vpf_offset-0.25, -1]], wp_yaw, carla=True)
        #     RIGHT_ADDED = True

        # if the right or left lane is crossable, set the opposite lane as non-crossable
        if RIGHT_ADDED == False:
            if wp.is_junction:
                pass
            # if wp.left_lane_marking.type in rcrossable_list and np.abs(wp_yaw - current_state[2]) < np.pi/6:
            else:
                self._model.solver_add_nc_road_pf([[right_bound, -1]], wp_yaw, carla=True)
                RIGHT_ADDED = True
                apf_nc_road += self._model.nc_road_pf([[right_bound, -1]], ref_traj, wp_yaw)
                # print('left lane is crossable, set the right lane as non-crossable')
        if LEFT_ADDED == False:
            if wp.is_junction:
                pass
            # if wp.right_lane_marking.type in lcrossable_list and np.abs(wp_yaw - current_state[2]) < np.pi/6:
            else:
                self._model.solver_add_nc_road_pf([[left_bound, 1]], wp_yaw, carla=True)
                LEFT_ADDED = True
                apf_nc_road += self._model.nc_road_pf([[left_bound, 1]], ref_traj, wp_yaw)
                # print('right lane is crossable, set the left lane as non-crossable')
        #     apf_nc_road += self._model.nc_road_pf([[right_bound-vpf_offset-0.25, -1]], ref_traj, wp_yaw)
        # Don't need to add the PF for the unknown lane if there is not a intersection
        # elif wp.right_lane_marking.type == carla.LaneMarkingType.NONE:
        #     # get the waypoint the vehicle is following
        #     # waypoint_now = ref_traj[:,0]
    ...
    def check_traffic_light_green(self, traffic_distance=15.0):
        if self._last_traffic_light:
